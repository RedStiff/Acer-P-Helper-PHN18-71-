...
namespace PredatorControlApp
{
    ...
    public class FanCurveGraphControl : Control
    {
        ...
        private (int temp, int fan) PixelToPoint(Point pt)
        {
            ...
            return (Math.Clamp((int)Math.Round(temp), 0, MaxTemp), Math.Clamp((int)Math.Round(fan), 0, 100));
        }
        ...
        private void DrawAxes(Graphics g)
        {
            ...
            for (int t = 0; t <= MaxTemp; t += _layout.XTickStep)
            {
                ...
                float lx = Math.Clamp(x - size.Width / 2f, 0, Width - size.Width);
            ...
            {
                float y = FanToY(f);
                g.DrawLine(axisPen, plot.Left - 3, y, plot.Left, y);
        ...
        private void DrawLiveMarker(Graphics g)
        {
            ...
            int fan = FanCurveEvaluator.Evaluate(_config, liveTemp.Value);
        ...
        private void DrawTooltip(Graphics g)
        {
            ...
            string text = $"{p.Temperature}°C, {p.FanPercent}%";
